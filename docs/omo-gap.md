...
## Closed this spiral (v1.1.41) — Hashline read-prefix strip

| Item | Behavior |
|------|----------|
| **ASCII `N->`** | 与 unicode `N→` 同等剥离 |
| **leading ws** | 行首空白 + LINE#ID / read 前缀仍识别 |
| **why** | paste 工具输出时假 stale PreTool deny |

## Closed this spiral (v1.1.42) — nested toolInput unwrap

| Item | Behavior |
|------|----------|
| **unwrapToolInput** | `arguments` / `parameters` / `input` 嵌套展平到 tool 参数 |
...
## Closed this spiral (v1.1.44) — shell write bypass round 2

| Item | Behavior |
|------|----------|
| **git clean/restore** | 读/写盘破坏性 git 操作 → mutating |
| **tar -x / unzip / rsync / dd of=** | 解压/同步/裸写 → mutating（`tar -t` 仍允许） |
| **bun/deno/composer install** | 包安装 → mutating |

## Closed this spiral (v1.1.45) — deferred DONE + package shell
...
## Closed this spiral (v1.1.47) — nextest/e2e verify + infra shell
...
## Closed this spiral (v1.1.52) — format-write / wrangler-tofu / lint verify / idle

| Item | Behavior |
|------|----------|
| **shell mutators** | prettier/eslint/biome --write · wrangler/tofu/sls · Expand-Archive · irm\|iex · gunzip/7za · psql -f · gh pr merge · perl -pi · php file_put_contents |
| **VERIFY_SHELL** | terraform validate/fmt-check · shellcheck/hadolint · rubocop/mix credo · format:check · govulncheck · deno check · opa/conftest |
...
## Closed this spiral (v1.1.54) — PowerShell tools + codemod/scaffold shells

| Item | Behavior |
|------|----------|
| **isShellTool + hooks** | PowerShell / pwsh / Cmd / terminal_command / run_command / Exec |
| **codemod shells** | yq -i / sd / jscodeshift / ast-grep -U / fastmod / knip --fix |
| **scaffold/ops** | nx g / ng generate / nest g / rails generate / artisan make / docker start\|stop / pm2 / flyctl / gh secret |
| **tool-paths** | fileName / fullPath / fsPath / from-to / old_path-new_path |
| **hashline** | strip `L12:` prefixes |
| **VERIFY** | tsc … --noEmit / nx run *:test / pnpm -r test / gradlew check / detox\|maestro |

## Closed this spiral (v1.1.55) — provisional DONE + archives/gh + ceremony quotes

| Item | Behavior |
|------|----------|
| **isDoneMessage** | reject DONE for now / soft|effectively DONE / marked DONE / DONE-ish|(WIP) |
| **ceremony opener** | allow backticks / quotes / 【】 around slogan |
| **shell mutators** | zip/gzip/tar -c · reg/icacls · npm version · ncu -u · gh pr/release create · curl -T · aws s3 mb · fnm install |
| **VERIFY** | mvn -B test · pnpm --filter · yarn workspace test |
| **paths** | file:// URI · documentPath/resourcePath |
| **idle** | Later / Cheers / 不急 / 好滴 |

## Closed this spiral (v1.1.56) — provisional VERIFIED + deploy/publish + edit aliases

| Item | Behavior |
|------|----------|
| **isVerifiedMessage** | reject soft/effectively/for now/marked VERIFIED · 暂时/先算全部测试通过 |
| **hard MUTATING + hooks** | SearchAndReplace / RewriteFile / ModifyFile / ApplyDiff / Patch |
| **shell mutators** | terraform init · ansible · cdk/sam · cloud deploy · npm/cargo publish · rails/laravel new · codegen |
...
## Closed this spiral (v1.1.58) — ULW ceremony PreTool hard + shell/path/verify

| Item | Behavior |
|------|----------|
| **ULW ceremony PreTool** | `ulwCeremonyPreDeny` 未开场硬拦 Write/变异 shell；口号同轮放行；banner 鸣锣三步 |
| **shell mutators** | git config/stash/branch -M · npm pack/link · Bun.write · gh issue · sops/op · nx migrate |
| **tool-paths** | new_content/code/value/data · documentUri file:// |
| **VERIFY** | type-check / types:check / bun typecheck |

## Closed this spiral (v1.1.59) — shell/path/verify/idle/ceremony false-negs

| Item | Behavior |
|------|----------|
| **shell tools** | run_in_terminal / execute_command / run_pty (+ hooks) |
| **shell mutators** | bun add · git notes/lfs · vercel --prod · npm run db:* · dlx · nix · crontab · node -p write |
...
## Closed this spiral (v1.1.61) — path arrays/nested + shell/idle/ceremony/verify

| Item | Behavior |
|------|----------|
| **tool-paths** | files/paths arrays · args/input/parameters nest · vscode-file:// |
| **shell mutators** | pnpm patch · tsx -e · docker commit · pg_dump · make deploy · git bisect |
| **VERIFY / idle / ceremony** | yarn coverage · composer/ant/sbt test · sounds good/好了。 · 🚀/【开场】 |
| **spawn / MCP** | subagent/agent_role/selected_agent · mcp git commit |

## Closed this spiral (v1.1.62) — MCP write_query + shell/path/idle/ceremony/verify green

| Item | Behavior |
|------|----------|
| **isMutatingTool** | write_query · mcp git add/checkout · postgres execute |
| **shell mutators** | yarn global · npm exec create · julia/elixir · useradd · psql -d -c · mongo --eval |
...
## Closed this spiral (v1.1.64) — ULW stall circuit + researchOnly state

| Item | Behavior |
|------|----------|
| **maxUlwStall circuit** | 默认 8 轮连续空转 → 自动 cancel + STALL CIRCUIT OPEN（`0` 关闭） |
