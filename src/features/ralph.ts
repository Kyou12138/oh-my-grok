...
export function parseGoalsFromTask(task: string): string[] {
  ...
  if (t.includes(";")) {
    ...
    if (parts.length === 1 && /;\s*$/.test(t)) return parts;
  }
  if (t.includes("|")) {
    const parts = t
      .split("|")
...
function emptyState(
  ...
  return {
    ...
    ceremonyOpened:
      partial.ceremonyOpened ?? (partial.mode === "ralph" ? true : false),
  };
}
 ...
 */
export function hasUlwCeremonyOpener(msg?: string): boolean {
  if (!msg) return false;
  ...
  return (ULW_CEREMONY_OPENERS as readonly string[]).some(
    (o) => bare === o || bare.toUpperCase() === o.toUpperCase(),
  );
}
...
export function isDoneMessage(msg?: string): boolean {
  ...
  const NEGATED_DONE =
    /\b(?:not|never|without|no|cannot|can'?t|unable|impossible|refuse|refusing|missing|far\s+from|rarely|seldom|hardly|barely|scarcely|don'?t|doesn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|won'?t|wouldn'?t|shouldn'?t|couldn'?t|mustn'?t|haven'?t|hasn'?t|hadn'?t|ain'?t|didn'?t)\b[^.!\n]*(?:<promise>DONE<\/promise>|<promise>done<\/promise>|RALPH_DONE|ULW_DONE)/i;
  ...
  const HEDGED_BEFORE =
    /\b(almost|nearly|mostly|partially|roughly|will|going\s+to|gonna|plan(?:ning)?\s+to|intend(?:ing)?\s+to|should|must|need\s+to|about\s+to|pending|todo|skip(?:ping)?|wait(?:ing)?|before|later|soon)\b[^.!\n]{0,80}(?:<promise>DONE<\/promise>|<promise>done<\/promise>|RALPH_DONE|ULW_DONE)/i;
  ...
  if (
    /(?:未|没有|没|并非|不|无法|不能|没法|难以|稍后|待会|之后|以后|还要|尚未)[^。\n]{0,24}(?:ULW_DONE|RALPH_DONE|DONE)|(?:ULW_DONE|RALPH_DONE)[^。\n]{0,24}(?:未完成|还没|仍有|之后|以后)/.test(
      msg,
    )
  ) {
    return false;
...
export const VERIFY_SHELL_RE =
  /\b(npm\s+(test|audit|run\s+(test|ci|typecheck|lint|check|doctor|validate)(:\w+)?)|pnpm\s+(test|audit|run\s+test|typecheck|lint)|yarn\s+(test|audit|run\s+test|typecheck)|bun\s+(test|run\s+test)|deno\s+test|node\s+--test|tsx\s+--test|vitest|jest|mocha|ava|pytest|py\.test|python3?\s+-m\s+(pytest|unittest)|(?:python3?\s+)?manage\.py\s+test|poetry\s+run\s+pytest|uv\s+run\s+(pytest|ruff|mypy)|hatch\s+run\s+test|cargo\s+(test|nextest|clippy|audit)|cargo\s+fmt[^|&;\n]*--check|nextest\s+run|go\s+(test|vet)|gotestsum|ginkgo|staticcheck|dotnet\s+test|mvn\s+(test|verify)|gradlew?\s+test|make\s+(test|check)|just\s+(test|check)|task\s+(test|check)|turbo\s+(run\s+)?test|nx\s+test|nx\s+(run-many|affected)[^|&;\n]*\btest\b|lerna\s+run\s+test|playwright\s+test|cypress\s+run|tox|hatch\s+test|flutter\s+(test|analyze)|dart\s+(test|analyze)|phpunit|pest|php\s+artisan\s+test|rspec|rails\s+test|bin\/rails\s+test|rake\s+test|mix\s+test|sbt\s+test|lein\s+test|stack\s+test|cabal\s+test|bazel\s+test|zig\s+(?:build\s+)?test|crystal\s+spec|swift\s+test|xcodebuild\s+test|fastlane\s+tests?|ctest|meson\s+test|ninja\s+test|bats|shellspec|ng\s+test|ember\s+test|typecheck|tsc\s+(--noEmit|-b|--build)|vue-tsc|svelte-check|astro\s+check|oxlint|ruff\s+check|ruff\s+format\s+--check|black\s+--check|isort\s+--check|flake8|pylint|mypy|pyright|biome\s+(check|ci)|prettier\s+--check|eslint|lint|semgrep|bandit|pip-audit)\b/i;

/** echo/printf of test names is not verification evidence. */
const ECHO_LIKE_RE = /^(echo|printf|Write-Host|console\.log)\b/i;

export function isVerifyShellCommand(command?: string): boolean {
...
export function noteUlwShell(
  ...
  a.shells += 1;
  if (command) {
    a.lastPaths = [...new Set([`shell:${command.slice(0, 80)}`, ...a.lastPaths])].slice(
      0,
      12,
    );
  }
...
export function advancePhaseFromActivity(
  ...
  if (activity.shells > 0 && state.phaseReached.implement) {
    ...
    if (state.phase === "implement") state.phase = "verify";
  }
  return state;
}
...
export function processLoopStop(
  ...
  const noRwShell =
    activity.reads === 0 && activity.writes === 0 && activity.shells === 0;
  let stall = false;
  if (state.mode === "ulw") {
