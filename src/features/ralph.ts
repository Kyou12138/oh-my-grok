...
export function hasUlwCeremonyOpener(msg?: string): boolean {
  ...
  if (bang > 0 && bang < bare.length - 1) {
    ...
    if (rest && /^(goal|目标|—|-|–)/i.test(rest)) bare = head;
  }
  const norm = (s: string) =>
    s
      .replace(/[!！.。?？🚀⚡🔔🔥✨]+$/u, "")
      .trim()
      .toUpperCase();
  return (ULW_CEREMONY_OPENERS as readonly string[]).some((o) => {
    if (bare === o || norm(bare) === norm(o)) return true;
    const nb = norm(bare);
    const no = norm(o);
    if (!nb.startsWith(no)) return false;
    if (nb.length === no.length) return true;
...
export function processLoopStop(
  ...
  if (
    ...
    writeProgressLog(
      ...
      state,
      "stall-circuit",
      `stall circuit open at ${state.stallCount}/${maxStall}`,
    );
...
export function ulwStallCircuitReason(state: RalphState, maxStall: number): string {
  return [
    CEREMONY_BAR,
