...
const FLUFF =
  /^(ok|okay|sure|sure thing|yes|no|done|thanks|thank you|thx|ty|yep|yup|nope|cool|great|perfect|awesome|noted|alright|lgtm|sgtm|wip|continue|resuming|resume|all set|all good|ship it|shipping|you got it|can do|继续|好的|好的呀|好哒|好吧|嗯|嗯嗯|哦|行|行吧|可以|收到|明白|了解|知道了|懂了|懂|稍后|一会|没问题|了解了|着手处理|搞定|就这样|那就这样|\.{1,6}|…+|👍|✅)[.。!?！？]?$/i;
...
export function isIdleAssistantMessage(msg?: string): boolean {
  ...
  if (
    t.length < 48 &&
    /^(稍等|等一下|马上|即将|稍后继续|稍后回复|我先看看|我来看看|我看一下|我去查一下|我去看下|查一下|看一下|看下|查下|研究一下|分析一下|先这样|好的我继续|继续处理|马上处理|这就开始|正在处理|处理中|着手处理|先摸一下|排查中|调试中|开发中|进行中|等我一下|请稍候|请稍等|回头再说|我试试|试试看|试一下|测一下|跑一下|弄一下|搞一下|处理一下|改一下|修一下|马上就好|别急|稍安勿躁|莫急)/.test(
      t,
    )
  ) {
    return true;
