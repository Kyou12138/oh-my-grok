 ...
 */
const FLUFF = /^(ok|okay|sure|yes|no|done|thanks|thank you|继续|好的|嗯|哦|行|可以|收到|明白|了解|稍后|一会|\.{1,6}|…+|👍|✅)\.?$/i;
export function isIdleAssistantMessage(msg) {
    ...
    if (/^i('ll| will)\s+(continue|get (right )?on it|do that|proceed)\b/i.test(t) ||
