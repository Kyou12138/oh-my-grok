 ...
 */
const FLUFF = /^(ok|okay|sure|yes|no|done|thanks|thank you|thx|ty|yep|yup|nope|cool|great|perfect|awesome|noted|alright|继续|好的|好哒|嗯|嗯嗯|哦|行|可以|收到|明白|了解|稍后|一会|没问题|了解了|着手处理|搞定|\.{1,6}|…+|👍|✅)[.。!?！？]?$/i;
export function isIdleAssistantMessage(msg) {
    ...
    if (t.length < 48 &&
        /^(稍等|等一下|马上|即将|稍后继续|稍后回复|我先看看|我来看看|我看一下|我去查一下|我去看下|查一下|看一下|研究一下|分析一下|先这样|好的我继续|继续处理|马上处理|这就开始|正在处理|处理中|着手处理|先摸一下|排查中|调试中|开发中|进行中|等我一下|请稍候|请稍等|回头再说)/.test(t)) {
        return true;
    }
