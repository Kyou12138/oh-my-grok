 ...
 */
const FLUFF = /^(ok|okay|sure|yes|no|done|thanks|thank you|继续|好的|嗯|哦|行|可以|收到|明白|了解|稍后|一会|没问题|了解了|着手处理|\.{1,6}|…+|👍|✅)[.。]?$/i;
export function isIdleAssistantMessage(msg) {
    ...
    if (/^i('ll| will)\s+(continue|get (right )?on it|do that|proceed|take a look)\b/i.test(t) ||
        ...
        /^(looking into it|taking a look|checking now|one moment|one sec|hang on|stand by|acknowledged|sounds good|right away|coming right up|give me a moment|just a second|bear with me)[\s.]*$/i.test(t)) {
        return true;
    }
    // Chinese status fluff without paths/tools (v1.1.13 + v1.1.50 我来看看/处理中)
    if (t.length < 48 &&
        /^(稍等|等一下|马上|即将|稍后继续|我先看看|我来看看|我看一下|先这样|好的我继续|继续处理|马上处理|这就开始|正在处理|处理中|着手处理|先摸一下)/.test(t)) {
        return true;
    }
