 ...
 */
export function markSubagentChildFinished(input, cfg, role) {
...
export function isResultRecoveryTool(toolName) {
    ...
    return (n.includes("taskoutput") || // get_task_output, task_output, GetTaskOutput
 ...
 */
export function isInlineSubagentResult(toolOutput) {
    if (!toolOutput)
        return false;
    const t = toolOutput.trim();
    if (t.length < 80)
        return false;
    if (isSpawnAnnounceMessage(t.slice(0, 280)))
        return false;
    if (isSpawnResultRecoveredMessage(t))
        return true;
    // substantial body with code/path/structure signals (sync spawn payload)
    const evidence = /[\\/].+\.\w{1,8}\b/.test(t) ||
