...
export function sessionResumeSummary(input, cfg) {
    ...
    if (ralph?.active) {
        ...
        if (ralph.mode === "ulw") {
            lines.push("  **ULTRAWORK MODE ENABLED** — 开场仪式: first line `ULTRAWORK MODE ENABLED!` / `ULTRAWORK 模式已启动！` then goal + explore; skip opener → Stop CEREMONY INCOMPLETE; see `.omg/ulw-loop/CEREMONY.md`");
        }
    }
