...
export function isMutatingShellCommand(command) {
    ...
    if (/\bunzip\b/i.test(c) ||
        ...
        /\b(helm\s+(install|upgrade|uninstall|delete)|kubectl\s+(apply|create|replace|delete|patch|scale|rollout)|terraform\s+(apply|destroy)|pulumi\s+(up|destroy))\b/i.test(c) ||
