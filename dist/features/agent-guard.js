...
export function isMutatingShellCommand(command) {
    ...
    if (/\b(tee|truncate|rm|rmdir|unlink|del|erase|rd|sponge)\b/i.test(c) ||
        ...
        /\bgit\s+(add|commit|push|checkout|reset|rebase|merge|am|apply|cherry-pick|clean|restore|rm|mv|pull|submodule|worktree|switch|init|tag)\b/i.test(c) ||
        ...
        /\b(npm|pnpm|yarn)\s+(i|install|ci|uninstall|remove|publish|add|update|upgrade|up|create|version)\b/i.test(c) ||
    ...
    if (/\bunzip\b/i.test(c) ||
        ...
        /\b(heroku\s+(create|config:set|container:push)|fly\s+(secrets\s+set|apps\s+create)|railway\s+variables\s+set)\b/i.test(c) ||
        ...
        /\bsupabase\s+(db\s+(push|reset)|migration|functions\s+deploy|link|start|stop)\b/i.test(c) ||
        ...
        /\bgh\s+(secret\s+set|variable\s+set|workflow\s+run|pr\s+(create|merge|close|edit)|release\s+(create|delete|upload)|repo\s+(create|delete|edit|fork|sync)|gist\s+(create|delete|edit)|api\s+-X\s+(POST|PUT|PATCH|DELETE))\b/i.test(c) ||
        ...
        /\b(psql\s+-c|sqlite3\s+\S+\s+['\"]?(?:CREATE|INSERT|UPDATE|DELETE|DROP)|mongosh)\b/i.test(c) ||
    ...
    if (/\b(curl|wget)\b[^|&;\n]*\s(-o|--output|-O|-P|--directory-prefix)\b/i.test(c) ||
    ...
    if (/\b(node|nodejs|deno|bun|python3?|py)\b[^|&;\n]{0,60}\s(-e|--eval|-c)\b/i.test(c)) {
        if (/\b(writeFileSync|writeFile|appendFileSync|appendFile|createWriteStream|outputFileSync|outputFile)\b/i.test(c) ||
            /\bopen\s*\([^)]*['"]w/i.test(c) ||
