...
export function saveDiag(input, cfg, st) {
    writeJsonAtomic(diagPath(input, cfg), st);
}
export function markDirty(input, cfg, file) {
    if (!cfg.diagEnforce)
        return;
...
export function markVerified(input, cfg) {
    const st = loadDiag(input, cfg);
...
export function markSoftPrompted(input, cfg) {
    const st = loadDiag(input, cfg);
    st.softPrompted = true;
    saveDiag(input, cfg, st);
}
export function runDiagCommand(input, cfg) {
    const st = loadDiag(input, cfg);
    if (!cfg.diagCommand) {
...
export function isVerifiedMessage(msg) {
    ...
    const VERIFY_MARK = /(?:<promise>VERIFIED<\/promise>|OMG_VERIFIED|\ball tests passed\b)/i;
    const ZH_PASS_MARK = /(?:全部|所有)测试(?:均)?(?:已)?通过|测试(?:全部|均)(?:已)?通过/;
    const DEFERRED_BEFORE = /\b(will|going\s+to|gonna|plan(?:ning)?\s+to|intend(?:ing)?\s+to|should|must|need\s+to|about\s+to|pending|todo|skip(?:ping)?|wait(?:ing)?|before|later|soon)\b[^.!\n]{0,80}(?:<promise>VERIFIED<\/promise>|OMG_VERIFIED|\ball tests passed\b)/i;
    const DEFERRED_AFTER = /(?:<promise>VERIFIED<\/promise>|OMG_VERIFIED|\ball tests passed\b)[^.!\n]{0,80}\b(later|after|once|when|until|then|pending|todo|skip|wait|yet|soon|for\s+now|for\s+today|temporarily|temporary|wip)\b/i;
    const DEFERRED_ZH = /(?:稍后|待会|之后|以后|还要|尚未|暂时|先算)[^。\n]{0,24}(?:全部|所有)?测试|(?:全部|所有)测试(?:均)?(?:已)?通过[^。\n]{0,16}(?:之后|以后|later|暂时)|(?:稍后|待会|之后|暂时|先算)[^。\n]{0,16}(?:VERIFIED|OMG_VERIFIED)|(?:VERIFIED|OMG_VERIFIED)[^。\n]{0,16}(?:之后|以后|暂时)/i;
    const PROVISIONAL_BEFORE = /\b(soft|provisional(?:ly)?|functional(?:ly)?|effective(?:ly)?|temporary|consider|treat\s+as|marking|marked(?:\s+as)?|i(?:'ll|\s+will)\s+mark)\b[^.!\n]{0,80}(?:<promise>VERIFIED<\/promise>|OMG_VERIFIED|\ball tests passed\b)/i;
    const PROVISIONAL_AFTER = /(?:<promise>VERIFIED<\/promise>|OMG_VERIFIED)(?:-ish|\s*\(\s*wip\s*\)|\s+wip\b)/i;
    if ((VERIFY_MARK.test(msg) || ZH_PASS_MARK.test(msg)) &&
    ...
    if (/diagnostics clean/i.test(msg) &&
        !/\b(?:not|never|aren't|isn't)\b[^.!\n]{0,40}diagnostics\s+clean|diagnostics\s+(?:not|aren't)\s+clean/i.test(msg)) {
        return true;
    }
    ...
    const NEGATED_ALL_TESTS = /\b(?:not|never|without|cannot|can'?t|unable|impossible|refuse|refusing|missing|far\s+from|rarely|seldom|hardly|barely|scarcely|don'?t|doesn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|won'?t|wouldn'?t|shouldn'?t|couldn'?t|mustn'?t|haven'?t|hasn'?t|hadn'?t|ain'?t|didn'?t)\b[^.!\n]*\ball tests passed\b/i;
    const HEDGED_AFTER = /\ball tests passed\b[^.!\n]{0,80}\b(except|but|however|failing|failed|error|errors|broken|still\s+fail|for\s+now|for\s+today|temporarily|temporary|wip)\b/i;
    const HEDGED_BEFORE = /\b(almost|nearly|mostly|partially|roughly|soft|provisional(?:ly)?|effective(?:ly)?|temporary|consider)\b[^.!\n]{0,40}\ball tests passed\b/i;
    ...
    if (/(?:全部|所有)测试(?:均)?(?:已)?通过|测试(?:全部|均)(?:已)?通过/.test(msg) &&
 ...
 */
export function diagPreDeny(input, cfg) {
    if (!cfg.diagEnforce)
        return null;
    const st = loadDiag(input, cfg);
    if (!st.lastErrors?.trim())
        return null;
    return [
...
export function diagUserContext(input, cfg) {
    if (!cfg.diagEnforce)
        return "";
    const st = loadDiag(input, cfg);
    if (!st.needsVerify && !st.lastErrors)
        return "";
    return [
        "<OMG_DIAGNOSTICS>",
