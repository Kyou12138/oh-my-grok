...
export function serializeRalph(state) {
    return [
...
export function detectRalphCommand(prompt) {
    ...
    const ulw = p.match(/^\/ulw-loop(?:\s+["']?(.+?)["']?)?\s*$/i) ||
        p.match(/^\/ultrawork(?:\s+["']?(.+?)["']?)?\s*$/i) ||
        p.match(/^\/ulw(?:\s+["']?(.+?)["']?)?\s*$/i);
    if (ulw)
        return { action: "start-ulw", task: (ulw[1] || "ultrawork until fully done").trim() };
...
export function ralphStopReason(state) {
    const header = state.mode === "ulw"
