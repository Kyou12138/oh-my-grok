...
export function hashlinePreToolDeny(input, cfg) {
    ...
    const isMulti = toolNorm.includes("multiedit") ||
        (Array.isArray(input.toolInput?.edits) &&
            input.toolInput.edits.length > 0);
...
function hashlineDenyBatchEdits(input, cfg) {
    ...
    for (const batch of batches) {
        ...
        for (const item of batch) {
            if (!item || typeof item !== "object")
                continue;
            const o = item;
            const file = String(o.file_path ?? o.path ?? o.filePath ?? o.target_file ?? o.targetFile ?? "");
            if (!file)
                continue;
            ...
            if (!current)
                continue; // new file create entry
            const oldRaw = String(o.old_string ?? o.oldString ?? o.old_str ?? o.search ?? "");
            if (!oldRaw.trim()) {
                return [
