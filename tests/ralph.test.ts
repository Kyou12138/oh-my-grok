 ...
 * - parseGoalsFromTask 尾分号 / detectRalphCommand 连字符 / isVerifyShellCommand
 *   echo 边界是已知轻微缺陷,断言当前行为(契约锁定),不写期望修复的红测试。
 */
...
// ─── 2. parseGoalsFromTask(契约锁定当前行为) ────────────────────────
describe("parseGoalsFromTask 多目标解析(契约锁定)", () => {
  it("分号分隔 'a; b; c' → [a,b,c]", () => {
  ...
  it("数字标记单字符目标 '1) a 2) b' → ['1) a 2) b'](契约锁定:正则单字符吞并缺陷)", () => {
    // 真实行为:捕获组 ([^\d].+?) 要求 [^\d] 后至少 1 字符,单字符 goal 'a'
    // 无法独立成段 → matchAll 得 ['a 2) b'](length 1) → 落回 [t]。已知缺陷,本轮不修。
    expect(parseGoalsFromTask("1) a 2) b")).toEqual(["1) a 2) b"]);
    expect(parseGoalsFromTask("1. a 2. b")).toEqual(["1. a 2. b"]);
  });

  it("数字标记多字符目标 '1) alpha 2) beta' → [alpha,beta](正则在多字符时生效)", () => {
  ...
  it("尾分号 'a;' → ['a;'](契约锁定,当前不剥离尾分号)", () => {
    // 当前实现:split(';') 得 ['a',''],trim/filter 得 ['a'],length<2 落回 [t]
    // t 已 trim 为 'a;',故返回 ['a;']。这是已知轻微缺陷,本轮不修。
...
// ─── 4. isVerifyShellCommand(契约锁定) ───────────────────────────────
describe("isVerifyShellCommand 词边界(契约锁定)", () => {
...
// ─── 6. detectRalphCommand(契约锁定) ─────────────────────────────────
describe("detectRalphCommand(契约锁定,避免与 ulw-v2 重复)", () => {
  ...
  it("'ulw-stop please' → start-ulw 但 task='-stop please'(契约锁定连字符边界)", () => {
    // 当前实现:'/^\s*ulw\b/' 命中(\b 在 w 与 - 之间),replace 仅吃 'ulw',
    // 残留 '-stop please'。已知轻微缺陷,本轮不修。
    const r = detectRalphCommand("ulw-stop please");
    expect(r.action).toBe("start-ulw");
    expect(r.task).toBe("-stop please");
  });
});
